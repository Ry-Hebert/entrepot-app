...
export default function BTCFlower(props) {
  ...
  const buyFromSale = async (qty, price) => {
    ...
    try {
      ...
      var r = await api
        .canister("pk6rk-6aaaa-aaaae-qaazq-cai", "sale")
        .reserve(
          price,
          qty,
      ...
      while (true) {
        try {
          ...
          r3 = await api.canister("pk6rk-6aaaa-aaaae-qaazq-cai", "sale").retreive(paytoaddress);
  ...
  return (
    <>
      <div style={styles.main}>
        ...
        <div style={{width: "100%", height: 200, borderRadius:10,backgroundPosition: "top", backgroundSize: "cover",backgroundImage:"url('/collections/btcflower/banner.jpg')"}}></div>
        <h1>Welcome to the official BTC Flower sale</h1>
        </div>
          ...
          <>{(!saleOver && remaining > 0) ?
            <>
              ...
              {whitelist && Date.now() >= startTime && Date.now() < whitelistend ? 
                <>
                  <p><strong><span style={{fontSize:"20px",color:"black"}}>You are on the whitelist! You can mint 1 NFT at 3ICP for the first 12 hours only!</span></strong></p>
