...
export default function Opener(props) {
  ...
  return (<>
    {playOpen ? 
    <>
      <HoverVideoPlayer videoRef={hoverVideoRef} muted={false} volume={0.3} style={{backgroundColor:"black",position:"fixed",left:0,right:0,top:0,bottom:0,zIndex:1700}} focused={playOpen} loop={false} videoSrc="/opening.mp4" /> 
      <Button variant={"outlined"} onClick={showPack} color={"primary"} style={{position:"fixed", right:50,bottom:100,color:"white",borderColor:"white",zIndex:1800,fontWeight: "bold", margin: "20px auto" }}>Skip</Button>
    </>
    : ""}
    <Backdrop style={{backgroundColor:"rgba(0,0,0,.8)",zIndex:1600,color:"white"}} open={openerOpen}>
      ...
      <>
        ...
        {openerCards.map((a, i) => {
          return (<Flip  key={i} id={i} onFlip={onFlip} flipSubscriber={flipSubscriber} card={a} />)
        })}
        </Grid>
